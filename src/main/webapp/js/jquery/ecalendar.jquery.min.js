$.fn.ECalendar = function(config) {
    ...
    var _this = $(this), ECalendar, day, imonth, iday, iyear, ilast, itimebox, select, getFullYear, getMonth, getDate, weekday, getHour, getMinutes, oper, currenttime, x, y, mx, my, timestep = config.step, drag, input, dragnum, isinit = false, pointer, callback, formatTime = config.format ? config.format : "yyyy-mm-dd hh:ii", formatDate = config.format ? config.format : "yyyy-mm-dd", tmp = ['#ee3f2f', '#1bbc9b', '#297fb8', '#985aa6', '#e67f22', '#2d3e50', '#27ae61', '#7e8c8d'];
    ...
    if (config.skin === false)
        config.skin = Math.floor(Math.random() * tmp.length)
    ...
    ECalendar.mousedown(function(event) {
    ...
    select.children("li").click(function() {
        ...
        if (index === 0)
            setData(date);
        if (index === 1)
            setData(new Date(date.getFullYear(),date.getMonth(),date.getDate() + 7,date.getHours(),date.getMinutes()));
        if (index === 2)
            setData(new Date(date.getFullYear(),date.getMonth() + 1,date.getDate(),date.getHours(),date.getMinutes()));
        if (index === 3)
            setData(new Date(date.getFullYear(),date.getMonth() + 3,date.getDate(),date.getHours(),date.getMinutes()));
        if (index === 4)
            setData(new Date(date.getFullYear() + 1,date.getMonth(),date.getDate(),date.getHours(),date.getMinutes()));
        select.animate({
            "height": 0
        }, 200);
        if (callback)
            callback(input.val(), ECalendar);
    });
    day.on("mousedown", "li", function(e) {
        ...
        drag = $(this);
        if (config.type == 0) {
            li.addClass('activ').css({
                "overflow": "visible",
                "background": config.skin
            }).append('<div class="at12"><div class="pointer"></div></div>');
    ...
    $(document).mouseup(function() {
        if (drag) {
            if (drag[0].tagName == "LI") {
                var m = drag.text();
                setData(new Date(getFullYear,getMonth - 1,getDate,getHour,getMinutes));
                drag.removeAttr("data-settime").css({
                    "overflow": "hidden"
                }).children("div").remove();
                ...
                var m = drag.text();
                if (drag.hasClass("imonth"))
                    setData(new Date(getFullYear,m - 1,getDate,getHour,getMinutes));
                if (drag.hasClass("iyear")) {
    ...
    function init() {
        ...
        config.icon === true && _this.after('<svg t="1493098552635" class="icon" style="position: absolute;right:' + (_this.height() - 20) / 2 + 'px;top:50%;margin-top: -10px;" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1006" xmlns:xlink="http://www.w3.org/1999/xlink" width="20" height="20"><defs><style type="text/css"></style></defs><path d="M856.576 200.192v54.784h58.368V814.08H109.568V256.512h56.832v-56.32H51.2V870.4h921.6V200.192h-116.224z" fill="#666666" p-id="1007"></path><path d="M819.2 102.4h-204.288v255.488L819.2 358.4V102.4z" fill="#EF4848" p-id="1008"></path><path d="M563.712 664.064h256V716.8h-256zM563.712 512h256v51.2h-256zM461.312 200.192h102.4v56.32h-102.4zM205.312 664.064h256V716.8h-256zM205.312 512h256v51.2h-256z" fill="#666666" p-id="1009"></path><path d="M410.112 102.4H204.8v256l205.312-0.512V102.4z" fill="#EF4848" p-id="1010"></path></svg>');
    ...
    function setData(date) {
        ...
        if (ECalendar.data("date") != (getFullYear + "-" + getMonth + "-" + oneday)) {
            ...
            for (var i = 1; i <= max; i++) {
                if (i < oneday)
                    list += "<li class='other prve'>" + (prevcount - (oneday - i) + 1) + "</li>";
                    ...
                    list += "<li class='other next'>" + (i - (count + oneday) + 1) + "</li>";
            }
            day.html(list);
        }
        var now = new Date();
        ...
        ECalendar.data("date", (getFullYear + "-" + getMonth + "-" + oneday));
        ...
        var val = config.format.replace('yyyy', getFullYear).replace('mm', getMonth < 10 ? "0" + getMonth : getMonth).replace('dd', getDate < 10 ? "0" + getDate : getDate).replace('hh', getHour < 10 ? "0" + getHour : getHour).replace('ii', getMinutes < 10 ? "0" + getMinutes : getMinutes);
